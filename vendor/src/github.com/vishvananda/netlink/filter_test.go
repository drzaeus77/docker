...
func TestFilterAddDel(t *testing.T) {
	...
	_, ok := qdiscs[1].(*Ingress)
	if !ok {
		t.Fatal("Qdisc is the wrong type")
	}
	fd, err := BpfOpen("/run/bcc/foo/functions/ingress/fd")
	if err != nil {
		fd = 0
	}
	filter := &U32{
	...
	out, err := exec.Command("tc", "filter", "show", "dev", "foo", "parent", "ffff:").Output()
	fmt.Printf("%s\n", out)
