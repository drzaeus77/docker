...
func BpfOpen(path string) (fd int, err error) {
	raddr, err := net.ResolveUnixAddr("unix", path)
	if err != nil {
		return
	}
	sock, err := net.DialUnix(raddr.Net, nil, raddr)
	if err != nil {
		return
	}
	defer sock.Close()
	...
	b := make([]byte, 64)
	oob := make([]byte, 64)
	_, oobn, _, _, err := sock.ReadMsgUnix(b, oob)
	if err != nil {
		return
	}
	oob = oob[:oobn]
...
func FilterAdd(filter Filter) error {
	...
	if u32, ok := filter.(*U32); ok {
		...
		if u32.BpfFd > 0 {
			table := nl.NewRtAttrChild(actions, tabIndex, nil)
			tabIndex++
			...
			nl.NewRtAttrChild(aopts, nl.TCA_ACT_BPF_PARMS, bpf.Serialize())
			nl.NewRtAttrChild(aopts, nl.TCA_ACT_BPF_FD, nl.Uint32Attr(uint32(u32.BpfFd)))
			nl.NewRtAttrChild(aopts, nl.TCA_ACT_BPF_NAME, nl.ZeroTerminated("bpf1"))
		}
		if u32.RedirIndex != 0 {
			table := nl.NewRtAttrChild(actions, tabIndex, nil)
			tabIndex++
